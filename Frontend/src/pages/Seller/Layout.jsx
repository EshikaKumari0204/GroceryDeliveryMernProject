...
const Layout = () => {
  ...
  const dashboardicon=(<img clasName="w-20" src={assets.add_icon}></img>)
          ...
          <div className="flex"> <div className="md:w-64 w-16 border-r h-80vh text-base border-gray-300 pt-4 flex flex-col transition-all duration-300  ">
                {sidebarLinks.map((item,) => (
                    <NavLink to={item.path} key={item.name} end={item.path==='/seller'}
                       className={({ isActive }) =>
  `flex items-center  gap-3 py-2 px-1.5
  ${
    isActive
      ? "border-r-4 md:border-r-[6px] bg-indigo-500/10 border-amber-500 text-amber-500"
